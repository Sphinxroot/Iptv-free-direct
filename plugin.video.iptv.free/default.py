...
mysettings = xbmcaddon.Addon(id = 'plugin.video.iptv.free')
...
def main():
	add_dir('[B]<<<  Recherchez  >>>[/B]', 'searchlink', 99, icon, fanart)
	if len(online_m3u) > 0:	
		add_dir('[COLOR red][B]>> ONLINE M3U <<[/B][/COLOR]', u_tube, 2, icon, fanart)
	if len(local_m3u) > 0:	
		add_dir('[COLOR blue][B]>> LOCAL M3U <<[/B][/COLOR]', u_tube, 3, icon, fanart)
	if len(online_xml) > 0:	
		add_dir('[COLOR red][B]>> ONLINE XML <<[/B][/COLOR]', u_tube, 4, icon, fanart)
	if len(local_xml) > 0:	
		add_dir('[COLOR blue][B]>> LOCAL XML <<[/B][/COLOR]', u_tube, 5, icon, fanart)		
